        ...
        builder: (context) {
          if (auth.role == 'driver') {
            final activeTrips = state.trips
                .where((t) => t.status == 'in_progress' || t.status == 'scheduled')
                .toList();
            final currentTrip = activeTrips.isNotEmpty ? activeTrips.first : null;
            final upcoming = activeTrips.length > 1 ? activeTrips.skip(1).toList() : [];
            ...
            if (currentTrip != null) {
              currentTripData = {
                ...
                'drop': currentTrip.placesToVisit ?? '',
                'vehicle': 'Assigned Vehicle', // TODO: Get vehicle mapping
              };
            }
            ...
            return DriverDashboardView(
              ...
              onStartTrip: () {
                ...
                if (currentTrip != null) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Starting Trip...')));
              },
              onEndTrip: () {
                // Navigate to End Trip processing
                if (currentTrip != null) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ending Trip...')));
              },
              onTripTap: (trip) {
                // Detail view
              },
